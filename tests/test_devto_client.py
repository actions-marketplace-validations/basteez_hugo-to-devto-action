...
def test_retry_honors_retry_after_on_429(devto_responses, monkeypatch):
    sleeps: list[float] = []
    monkeypatch.setattr("crosspost.devto.time.sleep", lambda s: sleeps.append(s))
...
def test_401_raises_auth_rejected(devto_responses, monkeypatch):
    monkeypatch.setattr("crosspost.devto.time.sleep", lambda s: None)
    devto_responses.add(
